...
def main():
    ...
    timestamp_path = input("Enter output path and press Enter to start recording: ")
    os.makedirs(timestamp_path, exist_ok=True)
    image_path =os.path.join(timestamp_path, "image.png")
    t_path = os.path.join(timestamp_path, "timestamp.txt")
    #timestamp_path = file_path.replace('.bag', '_timestamp.txt')
    timestamp_file = open(t_path, 'w')
    timestamp_file.write("frame_index,frame_timestamp\n")
    #color_save_dir = "images"
    #os.makedirs(color_save_dir, exist_ok=True)  # 自动创建文件夹，已存在则不报错
    #DISPLAY_WIDTH = 1280
    #DISPLAY_HEIGHT = 720
