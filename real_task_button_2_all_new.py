...
# 用于预测的动作偏移量，可以帮助补偿延迟
# 设为0表示总是执行计划的第0步，设为1表示执行第1步...
ACTION_OFFSET = 0 

# --- 模型与设备配置 ---
CHECKPOINT_PATH = "/home/baishan/diffusion_policy_real/data/epoch=0040_120.ckpt"
DEVICE = 'cuda:0'
N_EXEC_STEPS = 8 
...
ORIGINAL_ACTION_POS_MIN = np.array([-7.1553345, -7.342087, -1.5568848, -0.02825711, -0.0317029, -0.02400684])
ORIGINAL_ACTION_POS_MAX = np.array([11.183899, 7.26947, 18.37915, 0.03801771, 0.04800448, 0.02071836])
...
class CameraManager:
    ...
    def preprocess_for_model_cv2(self, bgr_image, device):
        resized_image = cv2.resize(bgr_image, (self.model_width, self.model_height), interpolation=cv2.INTER_AREA)
        rgb_image = cv2.cvtColor(resized_image, cv2.COLOR_BGR2RGB)
...
class RobotController:
    ...
    def execute_action(self, current_eef_pose_matrix, action_6d):
        ...
        ret = self.arm.rm_movel(target_pose_list, 3, 0, 0, 1)
        #更改为非阻塞模式，立即返回
        #ret = self.arm.rm_movej_p(target_pose_list, 3, 10, 0, 0)
