...
from imagecollect.utils import frame_to_bgr_image
...
class CameraManager:
    ...
    def get_latest_frame_wrong(self):
        SAVE_PATH = ("/home/baishan/diffusion_policy_real/pics") 
        frames = self.pipeline.wait_for_frames(100)
        if not frames: return None
        color_frame = frames.get_color_frame()
        
        timestamp = time.time()
        image_filename1 = os.path.join(SAVE_PATH, f"observation_origined_{timestamp}.jpg")
        cv2.imwrite(image_filename1,color_frame)
    ...
    def get_latest_frame(self):
        ...
        if not color_frame:
            return None
        local_image_path = "/home/baishan/diffusion_policy_real/result/color_frame_37.png" # 示例路径
        bgr_image = cv2.imread(local_image_path)
        print(np.array(bgr_image))
        # ==================== 关键修正 ====================
        """
        # 1. 使用 frame_to_bgr_image 将原始帧转换为 BGR 格式的 NumPy 数组
        ...
        """
        # 2. 使用转换后的 bgr_image 进行保存
        #timestamp = time.time()
        #image_filename1 = os.path.join(SAVE_PATH, f"observation_origined_{timestamp}.png")
        #cv2.imwrite(image_filename1, rgb_image) # 现在保存的是正确的图像数据了

        # 3. 返回转换后的 bgr_image 供后续处理使用
        return bgr_image

    def preprocess_for_model(self, rgb_image, device):
        #resized_image = cv2.resize(rgb_image, (self.model_width, self.model_height), interpolation=cv2.INTER_AREA)
        img = Image.fromarray(rgb_image).convert("RGB")
        resized_image = img.resize((self.model_width, self.model_height), Image.ANTIALIAS)
        SAVE_PATH = ("/home/baishan/diffusion_policy_real/pics") 
        timestamp = time.time()
        image_filename1 = os.path.join(SAVE_PATH, f"observation_resized_{timestamp}.png")
        resized_image.save(image_filename1)
        #rgb_image = cv2.cvtColor(resized_image, cv2.COLOR_BGR2RGB)
        #print(resized_image.size)
        #rgb_float = np.moveaxis(np.array(resized_image), -1, 1).astype(np.float32) / 255.0
...
def main():
    # --- 定义图片保存路径，并在程序开始时创建这个文件夹 (此部分不变) ---
    SAVE_PATH = "saved_observations_resized" # <--- 为区分，建议换个新文件夹名
    if not os.path.exists(SAVE_PATH):
        os.makedirs(SAVE_PATH)
        print(f"已创建文件夹用于保存【缩放后】的观测图片: {SAVE_PATH}")
    # --- ... 省略其他不变的初始化代码 ... ---
