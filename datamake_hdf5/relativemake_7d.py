        ...
        for i, folder_name in enumerate(traj_folders):
            ...
            for j in range(num_samples):
                if j < num_samples - 1:
                    ...
                    # 2. 将旋转矩阵转换为四元数 [qx, qy, qz, qw]
                    r = Rotation.from_matrix(rotation_matrix)
                    quaternion = r.as_quat()

                    # 3. 合并为 7D 动作向量 [tx, ty, tz, qx, qy, qz, qw]
                    action_7d = np.concatenate((translation, quaternion))
                    relative_actions_7d.append(action_7d)
                else:
                    # 4. 对于最后一帧，动作是无平移 (0,0,0) 和无旋转 (四元数 0,0,0,1)
                    last_action = np.array([0, 0, 0, 0, 0, 0, 1], dtype=np.float32)
                    relative_actions_7d.append(last_action)
            
            # 将 7D 动作列表转换为Numpy数组并写入HDF5
            relative_actions_np = np.array(relative_actions_7d, dtype=np.float32)
...
if __name__ == '__main__':
    ...
    # 输出的 HDF5 文件名
    OUTPUT_HDF5_FILE = 'my_robot_dataset_relative_7d.hdf5'
