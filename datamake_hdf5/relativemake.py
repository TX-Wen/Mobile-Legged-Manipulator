            ...
            for j in range(num_samples):
                if j < num_samples - 1:
                    ...
                    try:
                        ...
                        print(f"警告: 轨迹 {folder_name} 的第 {j} 帧矩阵是奇异的，无法计算逆。使用单位矩阵代替。")
                        p_t_transformed_inv = np.identity(4, dtype=np.float32)

                    # 4. 计算相对动作
                    relative_action_matrix = p_t_transformed_inv @ p_t_plus_1_transformed
                    relative_actions.append(relative_action_matrix.flatten())
                else:
                    # 5. 对于最后一帧，使用单位矩阵作为动作
                    relative_actions.append(np.identity(4, dtype=np.float32).flatten())
            
            # 将相对动作列表转换为Numpy数组并写入HDF5
            relative_actions_np = np.array(relative_actions, dtype=np.float32)
...
if __name__ == '__main__':
    ...
    # 输出的 HDF5 文件名
    OUTPUT_HDF5_FILE = 'my_robot_dataset_relative.hdf5'
