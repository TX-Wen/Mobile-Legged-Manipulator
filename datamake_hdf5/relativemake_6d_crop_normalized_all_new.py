...
def create_robomimic_dataset(root_dir, output_path):
    ...
    for folder_name in traj_folders:
        ...
        for j in range(num_samples):
            ...
            if j < num_samples - 1:
                ...
                p_t_transformed = p_t
                p_t_plus_1_transformed = p_t_plus_1
                try:
        ...
        all_actions.append(relative_actions_np)
    ...
    action_min = np.min(all_actions_np, axis=0)
    action_max = np.max(all_actions_np, axis=0)
    
    print("\n" + "="*50)
    print("全局 6D 动作统计信息：")
    print(f"  - 全局最小值 (min): {action_min}")
    print(f"  - 全局最大值 (max): {action_max}")
    print("="*50 + "\n")
    ...
    with h5py.File(output_path, 'w') as f:
        ...
        # 存储所有维度的 min 和 max
        data_grp.attrs["action_min"] = action_min
        data_grp.attrs["action_max"] = action_max
        # === MODIFIED SECTION END ===
        ...
        for i, traj_data in enumerate(all_trajectory_data):
            ...
            # 应用归一化公式: 2 * (x - min) / (max - min) - 1
            # 这会将数据从 [min, max] 映射到 [-1, 1]
            # 该操作现在直接应用于整个 (num_samples, 6) 的数组
            normalized_actions = 2.0 * (unnormalized_actions - action_min) / action_range - 1.0
            ...
            image_list = []
            img_shape = None
            # This is a placeholder for the original image shape from the loop
            img_size_for_print = (0, 0, 0)
            ...
            for img_filename in traj_data["image_filenames"]:
                ...
                if os.path.exists(img_path):
                    with Image.open(img_path) as img:
                        # Storing original shape for the print statement
                        
                        img_size_for_print = np.array(img).shape
                    ...
                    if img_shape is None:
                        # Assuming target size if first image is missing
                        img_shape = (360, 640, 3)
            ...
            print(f"图像原始尺寸: {img_size_for_print}")
...
if __name__ == '__main__':
    ...
    OUTPUT_HDF5_FILE = 'realman_dataset_normalized_6d.hdf5' # 建议改名以区分
